...
app.get("/", (req, res) => {
  ...
  const sql = `
    ...
    WHERE a.roll IN (
      ...
      HAVING COUNT(DISTINCT k.keyword) = ${keywords.length}
    )
    GROUP BY a.roll, a.name;`;
...
app.post("/", (req, res) => {
  ...
  const sql = `
    INSERT INTO cse3100.alumni (roll, name, position, company, city, state, country, contacts, about)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) as pdata
  ...
  dbPool.getConnection((err, connection) => {
    if (err) {
      ...
      connection.query(sql, personArr, (error, results) => {
        connection.release();
        if (error) {
          console.error("Database query error:", error);
          res.sendStatus(500);
